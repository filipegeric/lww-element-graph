...
class LWWElementSet<T> {
    private val addSet = mutableMapOf<T, Int>()
    private val removeSet = mutableMapOf<T, Int>()
    ...
    fun mergeWith(otherSet: LWWElementSet<T>): LWWElementSet<T> {
        val resultSet = LWWElementSet<T>()

        resultSet.addSet.putAll(mergeSets(addSet, otherSet.addSet))
        resultSet.removeSet.putAll(mergeSets(removeSet, otherSet.removeSet))
